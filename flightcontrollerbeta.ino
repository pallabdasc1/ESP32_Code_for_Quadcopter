...
void flightControlTask(void *pvParameters)
{
    while(1)
    {
        ...
        */
        ...
        if (!isAutoTuningActive && xSemaphoreTake(motorMutex, (TickType_t)5) == pdTRUE) 
        {
...
void loadPIDGainsFromEEPROM()
{
    prefs.begin("pid_gains", true); // RO mode    
    // Angle
    PAngleRoll = prefs.getFloat("PAngleRoll", PAngle);
    IAngleRoll = prefs.getFloat("IAngleRoll", IAngle);
    DAngleRoll = prefs.getFloat("DAngleRoll", DAngle);    
    PAnglePitch = prefs.getFloat("PAnglePitch", PAngle);
    IAnglePitch = prefs.getFloat("IAnglePitch", IAngle);
    DAnglePitch = prefs.getFloat("DAnglePitch", DAngle);    
    // Rate
    PRateRoll = prefs.getFloat("PRateRoll", PRate);
    IRateRoll = prefs.getFloat("IRateRoll", IRate);
    DRateRoll = prefs.getFloat("DRateRoll", DRate);    
    PRatePitch = prefs.getFloat("PRatePitch", PRate);
    IRatePitch = prefs.getFloat("IRatePitch", IRate);
    DRatePitch = prefs.getFloat("DRatePitch", DRate);    
    PRateYaw = prefs.getFloat("PRateYaw", 0.1);
    IRateYaw = prefs.getFloat("IRateYaw", 0.03);
    DRateYaw = prefs.getFloat("DRateYaw", 0.0);    
    prefs.end();
    Serial.println("[EEPROM] PID gains loaded.");
}

void savePIDGainsToEEPROM() 
{
    prefs.begin("pid_gains", false);  // Namespace: "pid_gains", RW mode    
    // Angle PID
    prefs.putFloat("PAngleRoll", PAngleRoll);
    prefs.putFloat("IAngleRoll", IAngleRoll);
    prefs.putFloat("DAngleRoll", DAngleRoll);    
    prefs.putFloat("PAnglePitch", PAnglePitch);
    prefs.putFloat("IAnglePitch", IAnglePitch);
    prefs.putFloat("DAnglePitch", DAnglePitch);    
    // Rate PID
    prefs.putFloat("PRateRoll", PRateRoll);
    prefs.putFloat("IRateRoll", IRateRoll);
    prefs.putFloat("DRateRoll", DRateRoll);    
    prefs.putFloat("PRatePitch", PRatePitch);
    prefs.putFloat("IRatePitch", IRatePitch);
    prefs.putFloat("DRatePitch", DRatePitch);    
    prefs.putFloat("PRateYaw", PRateYaw);
    prefs.putFloat("IRateYaw", IRateYaw);
    prefs.putFloat("DRateYaw", DRateYaw);    
    prefs.end();
    Serial.println("[EEPROM] PID gains saved.");
}
...
void autoTunePIDTask(void *pvParameters) 
{
  while (1) 
  {
    ...
    if (ppmData[4] > 1700 && abs(AngleRoll) < 5 && abs(AnglePitch) < 5 && InputThrottle > 1200) 
    {
...
{
     float testP[] = {0.5, 1.0, 1.5, 2.0, 3.0, 4.0};
     float testI[] = {0.01, 0.03, 0.05, 0.07};
     float testD[] = {0.001, 0.01, 0.05, 0.09};
     PIDGains best = {0, 0, 0};
     float bestScore = -1e6;
     ...
     for (int p = 0; p < sizeof(testP) / sizeof(testP[0]); p++) 
     {
       for (int i = 0; i < sizeof(testI) / sizeof(testI[0]); i++) 
       {
         ...
         {
           float score = runPIDTest(testP[p], testI[i], testD[d], sensorValue, isAngle, label);
           if (score > bestScore) 
           {
               bestScore = score;
               best.P = testP[p];
...
float runPIDTest(float P, float I, float D, float *feedback, bool isAngle, string label) 
{
    float error = 0, prevError = 0, iTerm = 0, output = 0;
    float input = 0, score = 0;
    ...
    // Reset filter states to avoid bias
    KalmanAngleRoll = 0;
    ...
    {
        // Get new gyro and angle estimates
        gyro_signals();
        RateRoll -= RateCalibrationRoll;
        RatePitch -= RateCalibrationPitch;
        RateYaw -= RateCalibrationYaw;
        ...
        // Evaluate PID output without changing control
        if (isAngle) 
        {
            if(label=="Angle Roll")
            {
                error = InputRoll - KalmanAngleRoll;
                pid_equation(error, P, I, D, PrevErrorAngleRoll, PrevItermAngleRoll);
                output = PIDReturn[0];
                PrevErrorAngleRoll = PIDReturn[1];
                PrevItermAngleRoll = PIDReturn[2];
    
                // Score based on how close output matches desired input
                float angleError = fabs(error);
                score -= angleError;
            }
            else if(label =="Angle Pitch")
            {
                error = InputPitch - KalmanAnglePitch;
                pid_equation(error, P, I, D, PrevErrorAnglePitch, PrevItermAnglePitch);
                output = PIDReturn[0];
                PrevErrorAnglePitch = PIDReturn[1];
                PrevItermAnglePitch = PIDReturn[2];
                // Score based on how close output matches desired input
        ...
        else 
        {
            if(label=="Rate Roll")
            {
                ...
                float rateError = fabs(input - *feedback);
                score -= rateError;
                */
                // === Roll Axis PID ===
                errorRoll = InputRoll - RateRoll;
                iTermRoll += I_roll * errorRoll * 0.004;
                float dTermRoll = D_roll * (errorRoll - prevErrorRoll) / 0.004;
                outputRoll = P_roll * errorRoll + iTermRoll + dTermRoll;
                prevErrorRoll = errorRoll;
            }
            else if(label=="Rate Pitch")
            {
                // === Pitch Axis PID ===
                errorPitch = inputPitch - RatePitch;
                iTermPitch += I_pitch * errorPitch * 0.004;
                float dTermPitch = D_pitch * (errorPitch - prevErrorPitch) / 0.004;
                outputPitch = P_pitch * errorPitch + iTermPitch + dTermPitch;
                prevErrorPitch = errorPitch;
            }
            else if(label=="Rate Yaw")
            {
                // === Yaw Axis PID ===
                errorYaw = inputYaw - RateYaw;
                iTermYaw += I_yaw * errorYaw * 0.004;
                float dTermYaw = D_yaw * (errorYaw - prevErrorYaw) / 0.004;
                outputYaw = P_yaw * errorYaw + iTermYaw + dTermYaw;
                prevErrorYaw = errorYaw;
            }
        
            float rollScore = fabs(errorRoll);
            score -= (Score);  // lower error = higher score
                
        }
       delay(20); // 50 Hz evaluation   
    }
    return score; // Less error = higher score
}
void setup()  
{
